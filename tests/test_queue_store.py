...
def test_crear_ficha_con_rut_lo_hashea(db_path: Path) -> None:
    ...
    ficha = crear_ficha(db_path, "Juan", "Control", "23-07-2026", rut="12345678-9")
...
def test_en_correccion_vuelve_a_revision(db_path: Path) -> None:
    ...
    avanzar_estado(
        ...
        "u",
        "PEDIR_CORRECCION",
        metadata={"feedback": "agrega creatinina"},
    )
    f2 = avanzar_estado(
        db_path,
        f.id,
        EstadoFicha.EN_REVISION_USER,
        aprobador_id="sistema",
        accion="REGENERAR_DRAFT",
    )
    assert f2.estado == EstadoFicha.EN_REVISION_USER
...
def test_token_rechaza_si_expirado(db_path: Path) -> None:
    ...
    with queue_store.get_connection(db_path) as conn:
        ...
        conn.execute("UPDATE tokens SET expira_en = ? WHERE id = ?", (pasado, token.id))
