...
class TestFormatearEstratificacion:
    ...
    def test_dx_con_badges_se_formatean_con_lista(
        self, estrat_completa: dict
    ) -> None:
    ...
    def test_dx_con_problema_y_sin_codigo(
        self, estrat_completa: dict
    ) -> None:
        out = formatear_estratificacion(estrat_completa)
    ...
    def test_dx_con_codigo_y_descripcion(
        self, estrat_completa: dict
    ) -> None:
        out = formatear_estratificacion(estrat_completa)
    ...
    def test_dx_sin_badges_sin_corchetes(
        self, estrat_completa: dict
    ) -> None:
        out = formatear_estratificacion(estrat_completa)
        # Colelitiasis no tiene badges -> linea debe ser sin "[]"
        colelitiasis_line = next(
            (l for l in out.splitlines() if l.startswith("- Colelitiasis")),
            None,
        )
    ...
    def test_estructura_basica(
        ...
        idx_cronicos = next(
            i for i, l in enumerate(lineas) if "Diagnosticos cronicos" in l
        )
...
class TestGuardarNotaClinicaConEstratificacion:
    def test_incluye_seccion_cuando_hay_datos(
        ...
        assert "=== FIN ESTRATIFICACION ECICEP ===" in texto
        # Orden: ESTRATIFICACION va entre DIAGNOSTICOS y ACTIVIDADES
        idx_dx = texto.index("=== FIN DIAGNOSTICOS ===")
        idx_estrat = texto.index("=== INICIO ESTRATIFICACION ECICEP ===")
        idx_act = texto.index("=== INICIO ACTIVIDADES ===")
    ...
    def test_no_sobrescribe_archivo_existente(
        ...
        # Segunda escritura: debe ser None (no sobrescribe)
        ...
        assert out2 is None
        # El archivo sigue con el contenido de la primera
        texto = out1.read_text(encoding="utf-8")
        assert "primera" in texto
        assert "segunda" not in texto
...
class TestModuloCarga:
    ...
    def test_selectores_estrategicos_presentes(self) -> None:
        ...
        con el HTML de Rayen que paso Miguel."""
        assert cnc._ESTRAT_CARD_SELECTOR == ".stratification-card"
        assert cnc._ESTRAT_BADGE_SELECTOR == (
            ".stratification-card .badge.badge-warning.badge-pill"
        )
        assert cnc._ESTRAT_MODAL_TRIGGER_SELECTOR == (
            '[data-modal="activeDiagnosisModalOpen"]'
        )
    ...
    def test_bug_fix_badge_fuera_del_popover(self) -> None:
        ...
        # El JS SI debe buscar el badge con un selector global
        assert "span.badge.badge-warning.badge-pill" in cnc._ESTRAT_CARD_JS
    ...
    def test_se_abre_popover_antes_del_modal(self) -> None:
        ...
        assert idx_popover < idx_modal, (
            "El popover debe abrirse ANTES del modal. "
            "Si no, el link 'Ver todos' no es clickable."
        )
...
class TestFixEsGenerico:
    ...
    def test_no_matchea_grupos_fuera_de_rango(self) -> None:
        ...
        for texto in invalidos:
            assert patron.match(texto) is None, (
                f"No deberia matchear: {texto!r}"
            )
    ...
    def test_extractor_devuelve_vacio_sin_badge(self) -> None:
        ...
        driver.execute_script.return_value = {
            "grupo": None, "grupo_label": None, "fecha_inicio": None
        }
        resultado = extraer_estratificacion_ecicep(
            driver, logging.getLogger("test")
        )
    ...
    def test_extractor_sigue_si_modal_falla(self) -> None:
        ...
        driver.execute_script.side_effect = [
            {"grupo": "G2", "grupo_label": "Riesgo moderado",
             "fecha_inicio": "24 ago. 2026"},  # card JS
