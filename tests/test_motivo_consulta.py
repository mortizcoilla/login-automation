...
class TestSelectorJsMotivo:
    ...
    def test_js_no_busca_dentro_de_collapse_text_sub(self) -> None:
        ...
        Sesion 2026-09-16: el JS filtra los candidatos con
        el.closest('.collapse-text-sub') en vez de evitar el string —
        verificamos que ese filtro este presente."""
        import inspect
        import re
        ...
        js = match.group(1)
        # El JS debe EXCLUIR los containers dentro de .collapse-text-sub
        assert "closest('.collapse-text-sub')" in js, (
            "El selector del motivo debe excluir los .textoverflow-container "
            "que estan dentro de .collapse-text-sub (ahi vive la anamnesis)."
        )
...
class TestGuardarNotaClinicaConMotivo:
    ...
    def test_motivo_aparece_dentro_de_nota_clinica(self, tmp_path: Path) -> None:
        ...
        contenido_lines = [
            linea
            for linea in lines_bloque
            if linea.strip() and not linea.startswith("===") and not linea.startswith("=" * 5)
        ]
        assert (
            contenido_lines[0] == "> **Motivo de atencion:** ingreso sm en dupla  (no logrado)"
        ), (
    ...
    def test_sin_motivo_no_aparece_label_vacio(self, tmp_path: Path) -> None:
    ...
    def test_sin_motivo_no_contamina_anamnesis(self, tmp_path: Path) -> None:
        ...
        contenido = [linea for linea in bloque if linea.strip() and not linea.startswith("=")]
        assert contenido[0] == "LINEA 1 DE ANAMNESIS"

    def test_motivo_y_estrato_coexisten(self, tmp_path: Path) -> None:
    ...
    def test_motivo_sobrescribe_archivo_existente(self, tmp_path: Path) -> None:
        ...
        assert out2 is not None  # SIEMPRE escribe (regla 14:14)
        texto = out1.read_text(encoding="utf-8")
        assert "motivo v2" in texto
        assert "motivo v1" not in texto
        assert "segunda version" in texto
        assert "primera version" not in texto
