...
def _parsear_informe_basico(ruta: Path) -> list[dict]:
    """Lee el informe (basico o enriquecido) y devuelve lista de filas.

    Wrapper del parser unificado (src.informes.parser, Fase 4b): acepta
    5/7/8 columnas y aplica la disambiguacion REQ-043 para 8 columnas
    (el layout deprecado 17:26 ya no corrompe tipo/motivo). Edad y
    requerimientos SIEMPRE None: se re-derivan de la nota (REQ-041/042).
    """
    from src.informes.parser import parsear_filas_enriquecidas

    return parsear_filas_enriquecidas(ruta)
