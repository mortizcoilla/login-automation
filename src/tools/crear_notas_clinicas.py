...
def guardar_marker_extraccion_fallida(
    ...
    notas_dir: Path,
    panel_cargo: bool,
    extraccion_parcial: dict[str, Any] | None = None,
) -> Path:
    """Escribe un archivo .md MARKER (no es una nota clinica) cuando la
    extraccion de anamnesis fallo.

    Sesion 2026-09-16 13:42: la regla "anamnesis SIEMPRE existe" exige
    rechazar archivos sin anamnesis. PERO dejar la carpeta vacia es peor
    UX que tener archivos placeholder (problema que motivó el cambio
    original). Solución: escribir un archivo marker que documenta el
    fallo. NO es una nota clínica, NO tiene anamnesis (porque no se
    pudo extraer), pero indica a Yadira exactamente qué falló.
        ...
        for i, paciente in enumerate(pacientes, 1):
            ...
            try:
                ...
                try:
                    ...
                    if out_path is None:
                        logger.warning(
                            f"[crear_notas] {paciente.nombre}: archivo "
                            f"ya existia, saltado."
                        )
                        stats["saltados"] += 1
                        pinfo.estado = "skipped"
                    else:
                    ...
                    # Regla Yadira 2026-09-16: anamnesis SIEMPRE existe.
                    # Si llegamos aca, guardar_nota_clinica rechazo la
                    # nota. Pero para no dejar la carpeta vacia,
                    # escribimos un MARKER de extraccion fallida. NO
                    # es una nota clinica valida, solo documenta el
                    # fallo para que Yadira sepa que hay que re-correr.
                    stats["errores_extraccion"] = (
                        stats.get("errores_extraccion", 0) + 1
                    )
                    pinfo.estado = "extraccion_fallida"
                    pinfo.errores.append(
                        f"ValueError: {ve!r}"
                    )
                    ...
                    logger.warning(
                        ...
                        f"Re-correr cuando Rayen este estable."
                    )
