"""Corre el flujo diario completo (REQ-008) en el orden real 4 -> 5 -> 3 -> 6.

Uso:
    python flujo_diario.py
...
PASOS = [
    ...
    ("5/6", "Informe base de fichas abiertas", ["-m", "src.analysis.informe_fichas_abiertas"]),
    (
        "3/6",
        "Scrapear fichas del informe (login Rayen)",
        ["-m", "src.tools.crear_notas_clinicas", "--todos", "--user", "yadira"],
    ),
    ("6/6", "Enriquecer el informe", ["-m", "src.analysis.enriquecer_informe"]),
]
