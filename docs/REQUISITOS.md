...
| REQ-059 | Todas las rutas de productos (notas, info, anamnesis, examenes, crudos, adjuntos, fichas_generadas, informes_trazabilidad, analysis, screenshots, logs) son configurables desde `.env`: `DATA_DIR` reubica todo y cada directorio tiene override individual. Defaults relativos a la raiz del repo, para que migrar de PC no requiera editar nada. Fuente unica: `src/core/rutas.py`. | Sesion 2026-09-21 | VIGENTE | `src/core/rutas.py` | `test_core_rutas.py` (6) |
| REQ-060 | Cron del flujo diario: calendario semanal configurable por doctor en `config/calendario.json` (dias con o sin tilde, HH:MM local). UNA tarea de Windows dispara cada 30 min y `src/scheduler/runner.py` decide con el calendario si corre (cambiar horarios no toca el Programador de Tareas). Si el PC estaba apagado a la hora, corre en el primer disparo posterior. Estado en `data/scheduler_estado.json`: 1 corrida OK por dia; fallos reintenta hasta `max_intentos_por_dia` (default 3). La tarea se crea DESACTIVADA. | Sesion 2026-09-21 | VIGENTE | `src/scheduler/` (`calendario`, `runner`, `instalar`), `scripts/registrar_scheduler.ps1` | `test_scheduler_calendario.py` (21) |
| REQ-061 | El bot de Rubicita tiene 2 misiones: (1) archivar foto/PDF en `EXAMENES_CRUDOS_DIR`, (2) AUTOMATICAMENTE tras archivar, transcribir TODAS las imagenes del paciente con OCR y consolidarlas en UN unico `exam_<pac>_<fecha>.md` en `EXAMENES_DIR`. Motor OCR por `.env` `OCR_ENGINE`: opencode (default, `opencode/mimo-v2.6-flash-free` con vision, `src/examenes/ocr_opencode.py`), Gemini (`src/examenes/gemini_api.py`, requiere prepago) o z.ai (respaldo). El md es derivado: si la transcripcion falla el md previo NO se toca; si tiene exito, los previos del mismo paciente+fecha se reemplazan (sin _v2). Los PDF se archivan sin OCR (aviso al usuario). Un fallo de OCR nunca invalida el archivado. | Sesion 2026-09-21 | VIGENTE | `src/telegram_bot/services/consolidar_service.py`, `src/examenes/ocr_opencode.py`, `src/examenes/gemini_api.py`, `handlers/photo.py` (reusa `src/examenes/consolidar.py`) | `test_telegram_bot_consolidar.py` (7), `test_ocr_opencode.py` (5), `test_gemini_api.py` (6) |
...
## 9. Paso 9 — avisos Telegram del cron (REQ-076)
...
| REQ-080 | Personalidad y avisos Rubicita: (a) aviso de FICHAS ABIERTAS durante el cron — total + distribucion por tipo de atencion (orden descendente) en bloque monoespaciado; (b) saludo matutino L/M/J/V 8:00-8:29 y miercoles 11:00-11:29 (jornada tardia de Yadira) via el latido del cron (sin tarea nueva), una vez por dia; (c) tono felino de Rubicita ('pata con garra', mordiscos) en los mensajes. Un fallo de aviso nunca afecta la cadena. | Yadira 22-09-2026 | VIGENTE | `src/scheduler/avisos.py`, `runner._quizas_saludo_matutino` | `test_scheduler_avisos.py` |
