...
| ID | Requisito | Origen | Estado | Implementacion | Tests |
|----|-----------|--------|--------|----------------|-------|
| REQ-054 | Paso 8 (`src/tools/cargar_ficha.py`) es opt-in independiente: Yadira lo invoca manualmente con `--paciente` + `--fecha` o `--todos`. NO entra en la cadena diaria (REQ-008). | Yadira 21-09-2026 | VIGENTE | `src/tools/cargar_ficha.py` (CLI) | `tests/tools/test_cargar_ficha.py` (14) |
| REQ-055 | Origen del pegado: `data/fichas_generadas/ficha_<pac>_<fecha>.md` (paso 7 / Mortadelo). Si el archivo no existe, el paciente se registra como `skip: sin insumo` y se sigue con el siguiente. | Yadira 21-09-2026 | VIGENTE | `leer_ficha_generada`, `cargar_ficha_de_paciente` | `test_cargar_ficha.py::test_cargar_ficha_de_paciente_sin_insumo_es_skip` |
| REQ-056 | Limite del flujo compartido paso 3 / paso 8: el doble click que abre la ficha y la espera al `<li class="verticalnav-tab-active"><div>Atencion actual</div></li>`. A partir de ahi, paso 3 hace click y entra al panel de evaluacion; paso 8 NO hace click y pega en el editor asociado. | Yadira 21-09-2026 | VIGENTE | `src/rayen/flujos/apertura_ficha.py` + `src/rayen/escritura/editor_anamnesis.py` | `tests/rayen/flujos/test_apertura_ficha.py` (6) |
| REQ-057 | NO auto-enviar: la regla dura de aprobacion humana se preserva en paso 8. El modulo pega el texto en el editor de Rayen y se detiene; Yadira revisa visualmente y aprieta Guardar ella misma. | Yadira 21-09-2026 | VIGENTE | diseno en `cargar_ficha.py` + `editor_anamnesis.py` | — (cubierto por inspeccion de diseno: ninguna ruta de codigo invoca "save" / "guardar" en Rayen) |
| REQ-058 | Trazabilidad por corrida: `data/trazabilidad_carga/carga_<ts>.json` con timestamp, modo, args, resumen (total/ok/skip/error/pendiente_selector) y detalle por paciente (nombre, fecha, ficha_path, estado, motivo, tipo_editor, caracteres_pegados). | Yadira 21-09-2026 | VIGENTE | `_escribir_trazabilidad` en `cargar_ficha.py` | `test_cargar_ficha.py::test_escribir_trazabilidad_json_contiene_resumen` |
| REQ-059 | Selector concreto del editor de "Atencion actual" en Rayen: pendiente de inspeccion manual con Yadira (DevTools del browser autenticado). Mientras tanto, los pegadores especificos (`pegar_en_textarea`, `pegar_en_contenteditable`, `pegar_en_ckeditor`, `pegar_en_tinymce`) son stubs que lanzan `NotImplementedError`; la corrida se registra como `pendiente_selector`. | Sesion 21-09-2026 | PENDIENTE | `src/rayen/escritura/editor_anamnesis.py` | `test_editor_anamnesis.py` (9, todos con stubs) |
