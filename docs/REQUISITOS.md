...
| REQ-005 | NO inventar datos que no esten en `data/info_paciente/`, anamnesis o examenes. | AGENTS.md | VIGENTE | Los extractores solo transcriben lo leido de Rayen | — |
| REQ-006 | Sin emojis en codigo, system prompts ni documentacion tecnica. | AGENTS.md | VIGENTE | Convencion | — |
| REQ-007 | Paso 7 (Mortadelo: rellenar ficha) eliminado; sera reescrito desde cero. | Sesion 2026-09-17 | PENDIENTE | — (eliminado `src/mortadelo/`, `mortadelo_*`) | — |

## 2. Flujo y orden

| ID | Requisito | Origen | Estado | Implementacion | Tests |
|----|-----------|--------|--------|----------------|-------|
| REQ-008 | Cadena diaria obligatoria (orden real de dependencias 4→5→3→6): `actualizar_mes_actual yadira` → `informe_fichas_abiertas` → `crear_notas_clinicas --todos` → `enriquecer_informe`. Los 4 CLIs encadenados con `&&` producen el producto del dia. | Usuario (Miguel) 18-09-2026 | VIGENTE | CLIs en `src/analysis/` y `src/tools/` (paths estables) | Smoke Fase 6 + golden baselines `data/analysis/golden_baseline_refactor/` |
...
| REQ-027 | Respaldo de anamnesis cruda separado: `data/anamnesis/anam_<pac>_<fecha>.md` con frontmatter + motivo blockquote + anamnesis. Sobrescribir el respaldo es OK. | Yadira 16-09 15:14/15:27 | VIGENTE | `guardar_respaldo_anamnesis` | incluido (8) |
...
| REQ-037 | Sanitizacion del tipo de atencion: se eliminan prefijos de instrumento (ME, EN, PS, TO, NU, KT) que Rayen antepone. | Sesion 2026-08 (src.plantillas) | VIGENTE | `sanitizar_tipo` (actualizar_mes_actual y listar_iniciados; unificar en Fase 2) | `test_pancho_skills.py` (parcial) |
