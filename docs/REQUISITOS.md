...
| REQ-075 | Selector concreto del editor de "Atencion actual" en Rayen: pendiente de inspeccion manual con Yadira (DevTools del browser autenticado). Mientras tanto, los pegadores especificos (`pegar_en_textarea`, `pegar_en_contenteditable`, `pegar_en_ckeditor`, `pegar_en_tinymce`) son stubs que lanzan `NotImplementedError`; la corrida se registra como `pendiente_selector`. | Sesion 21-09-2026 | PENDIENTE | `src/rayen/escritura/editor_anamnesis.py` | `test_editor_anamnesis.py` (9, todos con stubs) |
