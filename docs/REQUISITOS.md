...
## 8. Paso 8 — cargar_ficha: pegar la ficha generada en Rayen

| ID | Requisito | Origen | Estado | Implementacion | Tests |
|----|-----------|--------|--------|----------------|-------|
| REQ-070 | Paso 8 (`src/tools/cargar_ficha.py`) es opt-in independiente: Yadira lo invoca manualmente con `--paciente` + `--fecha` o `--todos`. NO entra en la cadena diaria (REQ-008). | Yadira 21-09-2026 | VIGENTE | `src/tools/cargar_ficha.py` (CLI) | `tests/tools/test_cargar_ficha.py` (14) |
| REQ-071 | Origen del pegado: `data/fichas_generadas/ficha_<pac>_<fecha>.md` (paso 7 / Mortadelo). Si el archivo no existe, el paciente se registra como `skip: sin insumo` y se sigue con el siguiente. | Yadira 21-09-2026 | VIGENTE | `leer_ficha_generada`, `cargar_ficha_de_paciente` | `test_cargar_ficha.py::test_cargar_ficha_de_paciente_sin_insumo_es_skip` |
| REQ-072 | Limite del flujo compartido paso 3 / paso 8: el doble click que abre la ficha y la espera al `<li class="verticalnav-tab-active"><div>Atencion actual</div></li>`. A partir de ahi, paso 3 hace click y entra al panel de evaluacion; paso 8 NO hace click y pega en el editor asociado. | Yadira 21-09-2026 | VIGENTE | `src/rayen/flujos/apertura_ficha.py` + `src/rayen/escritura/editor_anamnesis.py` | `tests/rayen/flujos/test_apertura_ficha.py` (6) |
| REQ-073 | Guardado automatico (REVISADA 23-09-2026 por la usuaria: se presiona el boton Guardar del editor como parte del flujo; version original: Yadira guardaba manualmente). El modulo pega el texto, presiona Guardar y espera el cierre del editor; si el editor no cierra, el paciente queda como error. La revision clinica queda en la lectura que Yadira hace del informe de trazabilidad y del informe de fichas abiertas en OneDrive. | Sesion 23-09-2026 | VIGENTE | `editor_anamnesis.guardar_editor_anamnesis` + flujo en `cargar_ficha.py` | cubierto por tests del editor |
| REQ-074 | Trazabilidad por corrida: `data/trazabilidad_carga/carga_<ts>.json` con timestamp, modo, args, resumen (total/ok/skip/error/pendiente_selector) y detalle por paciente (nombre, fecha, ficha_path, estado, motivo, tipo_editor, caracteres_pegados). | Yadira 21-09-2026 | VIGENTE | `_escribir_trazabilidad` en `cargar_ficha.py` | `test_cargar_ficha.py::test_escribir_trazabilidad_json_contiene_resumen` |
| REQ-075 | Selector concreto del editor de anamnesis en Rayen (entregado por Yadira 22-09-2026, ejemplo Rosa Davila Luengo): click al lapiz `button[id^='anamnesis-edit-']` (aria-label Modificar) dentro de `li#anamnesis`; el pegado va al textarea `#historiaEnfermedad` (reemplazo total por JS con eventos input/change y verificacion de largo). `#motivoConsulta` NO se toca. Guardar (`.add-header-button`) jamas se presiona (REQ-073). | Sesion 22-09-2026 | VIGENTE | `src/rayen/escritura/editor_anamnesis.py` (`abrir_editor_anamnesis`, `pegar_en_editor`) | `test_editor_anamnesis.py` (6) |
| REQ-078 | El informe de fichas abiertas (paso 5) se copia a OneDrive al generarse (junto a los demas productos): es para leer. El archivo canonico queda en analysis junto a la DB SQLite (que SIEMPRE queda local: OneDrive puede corruptarla). | Yadira 22-09-2026 | VIGENTE | `src/informes/base.py` (copia al generar) | suite de informes |
| REQ-081 | Ciclo de sesion del paso 8: Rayen soporta 8 fichas abiertas por sesion; al llegar al limite se cierra el navegador y se vuelve a loguear. Entre pacientes se vuelve a 'Pacientes citados'. Flujo por paciente: pegar en #historiaEnfermedad -> Guardar (automatico, REQ-073) -> volver a la lista. | Sesion 23-09-2026 | VIGENTE | `src/tools/cargar_ficha.py` (`iterar_pacientes`), `editor_anamnesis.guardar_editor_anamnesis` | `test_cargar_ficha.py` (reciclaje) |
| REQ-082 | Ciclo de vida de las fichas generadas: tras cada cadena, las fichas que YA NO estan en el informe de fichas abiertas (pacientes cerrados) se MUEVEN a 'fichas archivadas' (OneDrive, configurable `FICHAS_ARCHIVADAS_DIR`) — historial completo, nada se borra. Guardia: sin informe fresco no se mueve nada. CLI manual: `python -m src.tools.archivar_fichas [--dry-run]`. | Yadira 25-09-2026 | VIGENTE | `src/tools/archivar_fichas.py`, wiring en `runner.py` | `test_archivar_fichas.py` (5) |
| REQ-084 | El informe enriquecido incluye columna `Mortadelo` (si/no): SI si la nota tiene CUALQUIER bloque ** mortadelo — aunque sus pedidos no caigan en las 3 keywords (REQ-077). El parser acepta informes de 9 columnas (compat: legacy 17:26 sigue parseando bien via desambiguacion Edad_decimal). | Yadira 25-09-2026 | VIGENTE | `src/informes/enriquecer.py`, `src/informes/parser.py` | `test_informes_enriquecer.py`, `test_enriquecer_informe.py` |
